        ...
        while (running)
        {
            ...
            switch (action)
            {
                case 1:  // SHOW ALL TASKS
                    ...
                    {
                        for (var i = 0; i < ToDoList.Count; i++)                 // be very careful to not add an equality (<=) in the condition because that creates an out of range index (last index is 1 smaller than the count)
                        {
